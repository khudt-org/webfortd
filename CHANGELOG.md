...
- **변경**: 스크립트 `FOLDERS`·`SOURCE_NAME`·`PREFIX`에서 단체협약 제거, docstring에 제외 사유 기록. `content/agreements/`는 그대로 둔다(webfortd 자체 자산).
- **검증**: `--dry-run` 358건(84·74·69·131), 드라이브 현재본과 일치.

## 2026-09-23 — 3차 검수 반영 재생성: 엔진 0.3.1 + 2층 정렬·정규화·데이터 + 3층 규칙 3건 (358건, 주소 변경 0)

- **배경**: 3차 검수(9/21 마감, 50건 중 문제 10건 + 쪽 오기 1건, 2차 재발 0건)의 지적이 전부 규칙 계층이라 자문 메모 `260923_위키문서_3차검수_결과분석_및_반영지시.md` §4대로 2층 → 3층 → 재생성 → 회귀 순으로 반영. 건별 판정은 `docs/regression-2026-09-review50.md` 3차 표. 위원장 결정대로 드라이브 스냅샷은 이 저장소에서 교체하지 않는다(자문 세션 몫).
- **엔진 hwpx-tomd 0.3.1**: 새 값이 없는 행(모든 칸이 세로 병합 연속·빈 시작 칸, 연속 칸 1개 이상) 제거. 초안 HWP의 「중증장애 기준」 표 둘째 행(24번)과 중부대 절차표 단계 행 13건이 두 번씩 나오던 원인. 처음엔 「모든 칸이 연속」만 봤는데 레이아웃용 빈 열이 함께 있으면 행이 살아남고 `--prune-empty` 뒤 똑같은 행만 남아, 빈 시작 칸까지 보도록 넓혔다.
- **2층 정렬(docparse `hwpx_enrich.py`)**: 2차 정렬(빈 쪽 채우기) 신설 — 1차 LIS가 건너뛴 쪽만, 8자 조각 과반이 앞뒤 정렬 쪽보다 뚜렷이 많이 맞는 창 안 한 쪽에 붙인다(첫 판은 「앞뒤 쪽에 조각 하나라도 있으면 제외」였는데 「장애인교원」류 공통 조각 때문에 거의 못 채워 완화). 제목 줄은 3쪽 넘게 나오는 문구 제외(부록 옆 탭 오매칭), 인쇄 번호 없는 쪽은 라벨 접두 없이 `pdfN`. 실측: 4종 마커 100/121/135/364 → 113/127/136/366쪽, 최종보고서 [부록 1-2]·[1-3] 시작 쪽 정정(355→356, 361→364, PDF 대조).
- **2층 정규화·데이터**: `normalize-paragraphs.py`가 원문자·「가.」·「(1)」 줄을 문단으로 분리(44·45번, 지원인력 86곳)하고 줄머리 `* `·`** `·`*** `를 `\*`로 이스케이프(24번: Markdown이 각주 기호를 목록 •로 바꿔 표 안 `*`와 연결이 끊겼다). 정본 규약: **줄머리 별표는 각주 기호, 목록은 `-`만**. 서식 전사 데이터(`hr.json`·`school.json`)의 별표 글머리 정리 + 쪽 주석 6건씩(PDF에 글자 층이 없는 이미지 서식 115~122·134~140쪽). **중부대 두 동의서는 1쪽짜리라 인사관리에서 복사된 「제3자 제공」 절 4건이 원본에 없어 삭제**(134·140쪽 실측). `hr_additions.json`: 초안 HWP에 없는 최종본 추가분에 27·28·29·42·79·80쪽 주석, 지능정보화기본법 묶음을 최종본 자리(79~80쪽, 점자법 조문 뒤)로 이동. `정본 수정 목록.csv` +2행(「있 는」 — 원본 PDF도 같아 오탈자, 중부대 「교실 에」 변환 오류. 4종 한 음절 분리 어절 전수 대조 결과 이 2건뿐).
- **3층(`decompose-source.ts`)**: ① 관련 페이지 = 상위 문서(부모 절 개요 페이지) → 하위 문서(개요 페이지의 자식) → 형제 문서. 형제만이면 평면 목록, 상위·하위가 있으면 `###` 소제목(22·44번 「한 문서 = 한 절」 오해 3회 반복의 구조적 해소, 블록 변경 63건·신규 2건) ② 제목 직전 한 줄 라벨 `[별지 …]`·`[서식 …]`·`[부록 …]`을 다음 문서 첫 줄로(34번, 8건). 결과 358건(84·74·69·131) 주소 불변, 쪽 필드 변경 21건, 본문 갱신 97건. 설계 `docs/DECOMPOSE_V2_DESIGN.md` §3.4·§3.4b.
- **조치 없음(사유 기록)**: 13·21번 표 안 `<br>` 줄바꿈(사이트·docx에서는 이미 분리, 검수자가 md를 텍스트로 열어 본 것), 25번 유관기관 표 정돈(원본 구조 편집이라 온새미 편집 후보로 수행사 전달).
- **리뷰(code-reviewer 서브에이전트, P1 0)**: P2 ① 5.5만 자 분할된 개요 페이지(`-ptN`)는 슬러그가 바뀌어 상위·하위 링크를 잃을 수 있던 잠재 경로(현 데이터 분할 0건) → 원 슬러그로 조회하도록 수정 ② docparse 2차 정렬에 회귀 테스트가 없던 것 → `tests/test_hwpx_enrich.py` 3건 신설. P3(경고 slug 빈 값, 줄머리 `*`=각주 전제의 일반성)은 기록만.
- **검증**: hwpx-tomd pytest 67, docparse pytest 3, unit 424 pass + 1 skip(`tests/decompose-source.test.ts` H절 5건 신설), `validate:content` 417건, lint 0 error, 새 마커 독립 검증(다음 본문 줄의 8자 조각이 그 PDF 쪽에 있는지 — 글자 층 없는 이미지 서식 12건 제외 전부 일치), 인사관리 마커 역행 0. 남은 빈 쪽(표 연속·간지)은 회귀 문서 「남은 빈 쪽」 표.
