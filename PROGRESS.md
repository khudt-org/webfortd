...
- **3차 검수 반영 재생성 완료(2026-09-23)**: 3차 검수(9/21, 문제 11건·2차 재발 0건)의 규칙 계층 지적을 엔진(hwpx-tomd 0.3.1)·2층 정렬·정규화·데이터·3층 규칙 3건(관련 페이지 상위·하위·형제, 서식 라벨 귀속, 원문자 문단)으로 반영해 4종 재생성. 4종 파생 **358건 주소 불변**(research 84·hr 74·jbu 69·staff 131), 건별 판정 `docs/regression-2026-09-review50.md` 3차 표, 변경 요지 CHANGELOG 2026-09-23. 2차(9/11) 이력은 CHANGELOG. **드라이브 미반영**: `2. 마크다운 정본/`·`3. 위키 문서/` 스냅샷·`문서 목록.csv`·`정본 수정 목록.csv`(로컬 `data/source-hwp/_work/`가 최신, 94행) 교체는 자문 폴더 세션 몫(지시 §5). **전부 draft** — 수행사 전달 뒤 webfortd 공개 여부는 별건 판정(§미결 결정, BACKLOG C9).
...
4. **수행사 전달(자문 세션)**: 3차 반영본을 드라이브에 반영(스냅샷 교체·목록 CSV·회귀 표 `3차 검수 50건 회귀 표.md`)하고 원본 PDF·마크다운 정본·위키 문서 3계층을 「갱신 완료」 통보로 전달. 4차 검수는 열지 않는다. 그 뒤 webfortd 공개 판정(BACKLOG C9)은 별건.
...
| 4종 358건 공개 여부·시점 | 3차 검수 반영 재생성 완료(9/23). 수행사 전달 뒤 webfortd 공개는 **별건 판정**(본 사업과 분리된 트랙) — 공개로 판정되면 절차·선행 조건 3건은 BACKLOG C9 | `docs/BACKLOG.md` C9 |
